...
def lidar_check(version,
    ...
    gs = mpl.gridspec.GridSpec(
        2, 6, width_ratios=(1, 1, 1, 2*rat, 2*rat, 2*rat))
        ...
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, pts, binimgs) in enumerate(loader):
            ...
            for si in range(imgs.shape[0]):
                ...
                for imgi, img in enumerate(imgs[si]):
                    ...
                    plt.sca(final_ax)
                    plt.plot(img_pts[si, imgi, :, :, :, 0].view(-1), img_pts[si, imgi,
                             :, :, :, 1].view(-1), '.', label=cams[imgi].replace('_', ' '))
...
def viz_model_preds(version,
    ...
    fH, fW = final_dim
    if version == 'custom':
        fig = plt.figure(figsize=(2*fW*val, (fH+fW)*val))
        gs = mpl.gridspec.GridSpec(2, 2)
    else:
        fig = plt.figure(figsize=(3*fW*val, (1.5*fW + 2*fH)*val))
        gs = mpl.gridspec.GridSpec(3, 3, height_ratios=(1.5*fW, fH, fH))
        gs.update(wspace=0.0, hspace=0.0, left=0.0,
    ...
    with torch.no_grad():
        for batchi, batch in enumerate(loader):
            ...
            for si in range(imgs.shape[0]):
                ...
                if version == 'custom':
                    for imgi, img in enumerate(imgs[si]):
                        ax = plt.subplot(gs[imgi, 0])
                        showimg = denormalize_img(img)
                        plt.imshow(showimg)
                        plt.axis('off')
                        plt.annotate(cams[imgi], (0.01, 0.92),
                                     xycoords='axes fraction')
                    ax = plt.subplot(gs[:, 1])
                    ax.get_xaxis().set_ticks([])
                    ax.get_yaxis().set_ticks([])
                    plt.imshow(out[si].squeeze(0), vmin=0,
                               vmax=1, cmap='Blues')
                    ...
                    for imgi, img in enumerate(imgs[si]):
                        ...
                        plt.imshow(showimg)
                        plt.axis('off')
                        plt.annotate(cams[imgi].replace(
                            '_', ' '), (0.01, 0.92), xycoords='axes fraction')
                    ax = plt.subplot(gs[0, :])
                    ax.get_xaxis().set_ticks([])
                    ax.get_yaxis().set_ticks([])
                ...
                imname = os.path.join(
                    output_folder, f'eval{batchi:06}_{si:03}.jpg')
                print('saving', imname)
                plt.savefig(imname)
                counter += 1
